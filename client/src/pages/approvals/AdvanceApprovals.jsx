...
import { getAdvanceApplications, reviewAdvanceApplication } from '../../services/salaryAdvanceService';
...
export default function AdvanceApprovals() {
  ...
  const fetchApplications = useCallback(() => {
    setLoading(true);
    getAdvanceApplications({ status: status || undefined, page, limit: 10 })
  ...
  const submitReview = async () => {
    try {
      await reviewAdvanceApplication(reviewTarget.application.id, reviewTarget.decision, comment);
      toast.success(`Salary advance application ${reviewTarget.decision}`);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Salary Advance Approvals</h1>
        <p className="text-sm text-gray-500">Review and decide on employee salary advance requests</p>
      </div>
      ...
      <Card className="p-4">
        ...
        {loading ? (
          ...
          <EmptyState title="No salary advance requests" description="Nothing matches this filter right now." />
        ) : (
      ...
      <Modal
        ...
        title={reviewTarget ? `${reviewTarget.decision === 'approved' ? 'Approve' : 'Reject'} Salary Advance` : ''}
        footer={
          <>
